...
abstract class WriteCkanGroupOrg<T extends CkanGroupOrg> extends WriteCkanTest {
    ...
    public void testCreateMinimal() {
        ...
        LOG.log(Level.INFO, "created "+ newEmpty().getClass() + " with id {0} in catalog {1}", new Object[]{retGroupOrg.getId(), JackanTestConfig.of().getOutputCkan()});
    }
    ...
    public void testCreateById() {
        ...
        T retOrg = create(groupOrg);
        assertEquals(groupOrg.getId(), retOrg.getId());
        assertEquals(groupOrg.getName(), retOrg.getName());
    }    
    ...
    public void testCreateWithDatasetsWithoutId(){
        ...
        try {
            ...
            Assert.fail("Shouldn't be possible to create an "+ newEmpty().getClass() + " with datasets withour ids!");
    ...
    @Test
    public void testCreateWithNonExistingDatasets(){
        T groupOrg = newName("test-org-" + UUID.randomUUID().getMostSignificantBits());
        CkanDataset dataset = new CkanDataset("test-dataset-"+UUID.randomUUID().toString());
        ...
        try {
            T retGroupOrg = create(groupOrg);
            Assert.fail("Shouldn't be possible to create an "+ newEmpty().getClass() + " with nonexisting datasets!");
        } catch (JackanException ex){
    ...
    public void testCreateMirror() {
        ...
        checkNotEmpty(retGroupOrg.getId(), "Invalid "+ newEmpty().getClass() + " id!");
    ...
    public void testCreateWrongName(String groupOrgName) {        

        try {
            T groupOrg = newName(groupOrgName);
            create(groupOrg);
            Assert.fail("Shouldn't be able to create "+ newEmpty().getClass() + " with wrong name " + groupOrgName);
        }
        catch (Exception ex) {
    ...
    public void testCreateDuplicateName() {
        ...
        try {
            create(groupOrg);
            Assert.fail("Shouldn't be able to create "+ newEmpty().getClass() + " with same name " + name);
        }
        catch (JackanException ex) {
    ...
    @Test
    public void testCreateDuplicateById() {
        ...
        try {
            create(groupOrg);
            Assert.fail("Shouldn't be able to create "+ newEmpty().getClass() + " with same id " + groupOrg.getId());
        }
        catch (JackanException ex) {
