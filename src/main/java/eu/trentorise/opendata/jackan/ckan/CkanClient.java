...
public class CkanClient {
    ...
    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(CkanClient.class);
    
    public static ObjectMapper getObjectMapper(){
        if (objectMapper == null){
            objectMapper = new ObjectMapper();
            objectMapper.setPropertyNamingStrategy(PropertyNamingStrategy.CAMEL_CASE_TO_LOWER_CASE_WITH_UNDERSCORES)
                        .configure(DeserializationConfig.Feature.FAIL_ON_UNKNOWN_PROPERTIES, false) // let's be tolerant
                        .configure(DeserializationConfig.Feature.USE_GETTERS_AS_SETTERS, false); // not good for unmodifiable collections, if we will ever use any
     ...
     * @param params list of key, value parameters. They must be not be url encoded. i.e. "id","laghi-monitorati-trento"
     * @return 
     */
    <T extends CkanResponse> T getHttp(Class<T> responseType, String path,  Object ... params){        
        try {
            ...
            String json = Request.Get(fullUrl).execute().returnContent().asString();
            T dr = getObjectMapper().readValue(json, responseType);
            if (!dr.success){
                // monkey patching error type              
                throw new JackanException("Reading from the catalog was not successful. Reason: " + CkanError.read(getObjectMapper().readTree(json).get("error").asText()));
            }            
            return dr;            
        } catch (Exception ex) {
     ...
     */
    synchronized ArrayList<String> getDatasetList(){   
     ...
     */
    synchronized ArrayList<String> getDatasetList(Integer limit, Integer offset){   
        return getHttp(DatasetListResponse.class, "/api/3/action/package_list", "limit", limit, "offset", offset).result;
    }    
    
    /**
     * id should be the alphanumerical id like 96b8aae4e211f3e5a70cdbcbb722264256ae2e7d. 
     * Using the mnemonic like laghi-monitorati-trento is discouraged.
     * Throws JackanException on error.     
     */
    synchronized CkanDataset getDataset(String id){
        return getHttp(DatasetResponse.class, "/api/3/action/package_show", "id", id).result;
    }
     ...
     * @return 
     */
    synchronized CkanResource getResource(String id){
     ...
     * @return 
     */
    synchronized ArrayList<CkanGroup> getGroupList(){
     ...
     * @return 
     */
    synchronized ArrayList<String> getGroupNames(){
     ...
     * @return 
     */
    synchronized CkanGroup getGroup(String id){
        return getHttp(GroupResponse.class, "/api/3/action/group_show", "id",id).result;
    }
    ...
    /**
     * Only tags containg string given in query will be returned
     * Throws JackanException on error.
     * @param query
     * @param vocabularyId 
     * @return 
     ...
     * @return 
     */
    synchronized ArrayList<String> getTagNamesList(){
...
class CkanError {
    ...
    /**
     * To overcome the __type problem. Tried many combinations but Jackson is not collaborating here, even if in Group.isOrganization case setting the JsonProperty("is_organization") did work. 
     * 
     * @return 
     */
